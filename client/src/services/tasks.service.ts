...
export async function getTasks(): Promise<WeddingTask[]> {
  const { data } = await api.get<ApiResponse<WeddingTask[]>>('/tasks');
  if (!data.success || !data.data) throw new Error(data.message ?? 'שגיאה בטעינת משימות');
  return data.data;
}

export async function createTask(payload: TaskFormData): Promise<WeddingTask> {
  const { data } = await api.post<ApiResponse<WeddingTask>>('/tasks', payload);
