 ...
 */
export function buildGuestUrl(fullName: string, lastFourDigits: string): string {
  const base = (import.meta.env.VITE_SITE_URL as string | undefined)?.replace(/\/$/, '')
    ?? window.location.origin;

  const params = new URLSearchParams({ n: fullName, p: lastFourDigits });
...
): { fullName: string; lastFourDigits: string } | null {
  const n = searchParams.get('n');
  const p = searchParams.get('p');
  if (!n || !p || !/^\d{4}$/.test(p)) return null;
  return { fullName: n, lastFourDigits: p };
}
