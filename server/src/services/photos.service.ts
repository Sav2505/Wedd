...
export async function getAllPhotos(): Promise<(Photo & { url: string })[]> {
  const { rows } = await pool.query<Photo>(
    `SELECT p.id, p.uploader_id, g.full_name AS uploader_name,
            p.url, p.caption, p.uploaded_at, p.mime_type,
...
export async function savePhoto(
  ...
  const { rows } = await pool.query<Photo>(
    `INSERT INTO photos (uploader_id, url, caption, thumbnail_data, full_data, mime_type)
